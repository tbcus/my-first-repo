# Overhead — live flight track projection (receiver edition)
...
- **Hardware-only data** — aircraft come straight from your local
  `dump1090`/`readsb` receiver (real RF, no internet needed). There is no
  internet API fallback; for development without hardware, use the built-in
...
```bash
# Built-in simulated traffic — fully offline, good for development and demos:
...
OVERHEAD_LAT=<your lat> OVERHEAD_LON=<your lon> python3 run.py
```
...
| `OVERHEAD_SOURCE` | `dump1090` | `dump1090` (local SDR receiver) or `demo` (built-in simulated traffic, fully offline). |
| `OVERHEAD_DUMP1090_URL` | `http://127.0.0.1:8080/data/aircraft.json` | Where the decoder publishes data. |
| `OVERHEAD_POLL_S` | 1.0 | Poll interval in seconds. |
